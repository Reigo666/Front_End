...
> 核心思想：`HTML`搭建结构，`CSS`添加样式，实现了：结构和样式的分离。
...
### 2.2内部样式
- 卸载`html`页面内部，将所有的`CSS`代码提取出来，单独放在`<style>`标签中。
- 语法：
```CSS
<style>
    h1{
...
2. 定位元素的显示层级比普通元素高，无论什么定位，显示层级都是一样的。`(z-index:0)`
&emsp;&emsp;默认规则是：
&emsp;&emsp;&emsp;&emsp;- 定位的元素会盖在普通元素之上。
&emsp;&emsp;&emsp;&emsp;- 都发生定位的两个元素，后写的元素会盖在先写的元素之上。
...
### 4.3粘性定位的作用方式
1. 当元素滚动到在距离参考点`left`、`right`、`top`、`bottom`的位置后，元素将固定到该位置。
2. 元素将在父元素被收走后一并收走。
...
## 6.定位的特殊应用
> 注意：
> 1. 发生**绝对定位、固定定位**后，元素都变成了定位元素，默认宽高被内容撑开，且依然可以设置宽高。
> 2. 发生**相对定位**后，元素依然是之前的显示模式。
> 3.以下所说的特殊应用，只针对**绝对定位和固定定位**的元素，**不包括相对定位**的元素。

**让定位元素的宽充满包含块(不能设置wh)**
1. 块宽度与包含块一致，可以给定位元素同时设置`left`和`right`为0.
2. 块高度与包含块一致，`top`和`bottom`设置为0.
...
### 方案三：Normalize.css
`Normalize.css`是一种最新方案，它在清楚默认样式的基础上，保留了一些有价值的默认样式。
...
> 备注：`Normalize.css`的重置，和`reset.css`相比，更加的温和，开发时间可根据实际情况进行选择。
